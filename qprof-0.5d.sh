...
#---------------------------------------------------------------------------
QPV="qprof version 0.5d - Sep 2026"
OUT=qprof.out
CLC=0
GZP=0
RPOOL=""
...
cat <<-EOF | ${VSQL} -X -q -P null='(null)' -o ${OUT} -f -
    ...
    -- ------------------------------------------------------------------------
    \echo '    Step 17: Query execution profile counters extraction in TXT, tab delimited, format'
    \qecho >>> Step 17: Query execution profile counters extraction in TXT, tab delimited, format
